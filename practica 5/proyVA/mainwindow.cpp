...
void MainWindow::compareCorners(std::vector<std::vector<float>> puntosIzda, std::vector<std::vector<float>> puntosDcha){
    ...
    puntosD.clear();
    puntosI.clear();
    std::cout<< "start with the loop: " << puntosIzda.size() << std::endl;
    for(int i = 0; i < puntosIzda.size(); i++){
        ...
        for (int j = 0; j < aux.cols; j++) {
            ...
            //std::cout<< "Indice i: " << i <<" j: " << j <<" sacamos pDcha" << std::endl;
            if(aux.at<uchar>(pDcha)==1 and puntosIzda[i][1] >= w/2 and puntosIzda[i][0] >= w/2 and pDcha.x >= w/2 and pDcha.y >= w/2
              ...
              //  qDebug()<< "pDcha es punto x: " << pDcha.x << " y: " << pDcha.y;
                ...
                //qDebug()<< "sacado el template" ;

                if(result.at<float>(0,0) > best /*and result.at<float>(0,0) > 0.8*/){
